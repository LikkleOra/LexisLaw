    ...
    <Card shadow className="bg-[#f4f4f4] border-lexis-green border-l-4 p-8 md:p-12 space-y-12">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="space-y-2">
          <span className="font-mono text-xs uppercase tracking-widest text-[#333333]">Active Matter</span>
          <h3 className="text-4xl font-display tracking-tight text-black">{matter.reference}</h3>
        </div>
        <div className="flex flex-wrap gap-4">
          ...
          {matter.wa && (
            <a 
              ...
              target="_blank" 
              className="flex items-center gap-2 bg-lexis-green/20 text-lexis-green border border-lexis-green px-4 py-1 font-mono text-xs uppercase tracking-widest hover:bg-lexis-green hover:text-black transition-all"
            >
              WhatsApp Assistant
            </a>
          )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {infoItems.map((item, idx) => (
          <div key={idx} className={`space-y-3 p-6 border-l-2 ${item.highlighted ? 'bg-lexis-red/5 border-lexis-red' : 'bg-lexis-black/5 border-black/10'}`}>
            <div className="flex items-center gap-2 text-[#333333]">
              <item.icon size={14} />
              <span className="font-mono text-[10px] uppercase tracking-widest">{item.label}</span>
            </div>
            <p className={`font-mono text-sm uppercase tracking-tight ${item.highlighted ? 'text-lexis-red font-bold' : 'text-black'}`}>
              {item.value}
            </p>
          </div>
        ))}
