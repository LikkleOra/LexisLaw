  ...
  return (
    <div className="w-full pt-12 pb-8">
      <div className="flex justify-between items-center relative">
        {/* Connection Line */}
        <div className="absolute top-1/2 left-0 w-full h-[2px] bg-lexis-black/10 -translate-y-1/2 -z-10" />
        <div 
            ...
            <div key={idx} className="flex flex-col items-center gap-4 group">
              <div 
                className={`w-6 h-6 border-2 flex items-center justify-center transition-all duration-300 ${
                  ...
                  isCurrent ? 'bg-white border-lexis-red shadow-brutal-red animate-pulse' : 
                  'bg-white border-black/10'
                }`}
              />
              <span 
                className={`font-mono text-[9px] uppercase tracking-widest text-center max-w-[80px] transition-colors ${
                  isCurrent ? 'text-lexis-red font-bold' : isCompleted ? 'text-black' : 'text-[#333333]'
                }`}
              >
                {step}
              </span>
