      ...
      <main className="min-h-screen bg-white flex items-center justify-center p-6">
        <Card shadow className="w-full max-w-md p-12 bg-[#f4f4f4] border-black/10 space-y-12 animate-slam-in">
          <div className="text-center space-y-4">
             ...
             <p className="font-mono text-xs text-[#333333] uppercase tracking-widest leading-relaxed">
      ...
      <div className="flex-1 ml-72 p-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {[
            ...
            { label: 'Active Clients', value: '184', icon: LucideUsers, color: 'text-black' },
            { label: 'Pending Bookings', value: '05', icon: LucideCalendar, color: 'text-lexis-red' },
            { label: 'Closed Cases', value: '24', icon: LucideFileText, color: 'text-black' },
          ].map((stat, idx) => (
            <Card key={idx} className="bg-[#f4f4f4] border-black/5 p-8 border-l-4 border-l-lexis-red shadow-brutal hover:translate-x-1 hover:-translate-y-1 transition-all group">
              <div className="flex justify-between items-start mb-6">
                 <div className="w-10 h-10 bg-lexis-black/5 flex items-center justify-center border border-black/10 text-lexis-red">
                    <stat.icon size={20} />
                 </div>
              ...
              <div className="space-y-1">
                 ...
                 <div className="font-mono text-[9px] text-[#333333] uppercase tracking-[0.2em]">{stat.label}</div>
              </div>
            </Card>
          ))}
        </div>
        ...
        {currentView === 'bookings' && (
          <Card className="bg-[#f4f4f4] border-black/10 overflow-hidden" padding="none" shadow>
            <div className="p-8 border-b border-black/5 flex justify-between items-center">
               <h3 className="font-display text-xl tracking-tight text-black uppercase">Recent Consultations</h3>
               <span className="font-mono text-[10px] uppercase tracking-widest text-[#333333]">Showing last 10 entries</span>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left font-mono text-xs">
                <thead>
                  <tr className="bg-black/40 border-b border-black/5">
                    <th className="p-6 uppercase tracking-widest text-[#333333] font-bold">Reference</th>
                    <th className="p-6 uppercase tracking-widest text-[#333333] font-bold">Client Name</th>
                    <th className="p-6 uppercase tracking-widest text-[#333333] font-bold">Matter Type</th>
                    <th className="p-6 uppercase tracking-widest text-[#333333] font-bold">Preferred Date</th>
                    <th className="p-6 uppercase tracking-widest text-[#333333] font-bold">Status</th>
                    <th className="p-6 uppercase tracking-widest text-[#333333] font-bold">Action</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/5">
                  {[
                    ...
                    <tr key={idx} className="hover:bg-lexis-black/[0.02] transition-colors group">
                      <td className="p-6 text-lexis-red font-bold">{row.ref}</td>
                      <td className="p-6 text-black">{row.name}</td>
                      <td className="p-6 text-[#333333] uppercase">{row.type}</td>
                      <td className="p-6 text-[#333333]">{row.date}</td>
                      <td className="p-6">
        ...
        {currentView === 'matters' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {[
              ...
              <Card key={idx} shadow className="bg-[#f4f4f4] border-black/10 p-8 space-y-6 group hover:border-lexis-red transition-all">
                <div className="flex justify-between items-start">
                   <div className="space-y-1">
                      <span className="font-mono text-[9px] uppercase tracking-widest text-[#333333]">Active Case</span>
                      <h4 className="text-2xl font-display tracking-tight text-black group-hover:text-lexis-red transition-colors">{matter.ref}</h4>
                   </div>
                   <Badge variant={matter.progress === 4 ? 'verified' : 'progress'} size="sm">
                     Stage {matter.progress}/4
                   </Badge>
                </div>
                <div className="grid grid-cols-2 gap-4 border-t border-b border-black/5 py-4 font-mono text-[10px] uppercase tracking-widest text-[#333333]">
                   <div>
                ...
                <div className="w-full h-1 bg-lexis-black/5 overflow-hidden">
                   <div 
                ...
                <div className="flex justify-between items-center pt-2">
                   <span className="font-mono text-[9px] text-[#333333] uppercase tracking-widest">Update status</span>
